...
mp.add_key_binding(nil, "select-sid", function ()
    select_track("sid", "sub", "选择主字幕:", "无可用字幕")
end)
...
mp.add_key_binding(nil, "select-aid", function ()
    select_track("aid", "audio", "选择音频轨道:",
                 "无可用音频轨道")
end)

mp.add_key_binding(nil, "select-vid", function ()
    select_track("vid", "video", "选择视频轨道:",
                 "无可用视频轨道")
end)
...
local function format_edition(edition)
    return edition.title or ("版本 " .. edition.id + 1)
end
...
mp.add_key_binding(nil, "menu", function ()
    ...
    local menu = {
        {"主字幕", "script-binding select/select-sid", sub_track_count > 0},
        ...
        {"支持", "script-binding select/open-chat", true},
    }
...
local function editions()
    ...
    for i, edition in ipairs(get("user-data/edition-list", {})) do
        items[i] = {
