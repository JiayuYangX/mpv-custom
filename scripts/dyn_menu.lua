...
local function build_history_title(item)
    ...
    local hint = ""
    local title = ""
    if item.path:find("://") then
        local scheme = item.path:match("^([%a[%w.+-]-)://")
        title = abbr_title((item.title and item.title ~= "") and item.title or item.path)
        hint = (scheme and scheme:upper() or "URL") .. " | " .. time
    else
        local _, filename = utils.split_path(item.path)
        local name, ext = filename:match("^(.*[^%.])%.([^%.]+)$")
        title = abbr_title((name and name ~= "") and name or filename)
...
local function update_history_menu(menu)
    ...
    if current_path then
        for i = #history, 1, -1 do
            if history[i].path == current_path then
    ...
    for id = from, to do
        ...
        if item then
            local title, hint = build_history_title(item)
            append_menu(submenu, {
                title = title,
                shortcut = hint,
    ...
    if to < #history then
        append_menu(submenu, {
            title = '...',
            shortcut = string.format('[%d]', #history - to),
            cmd = 'ignore',
        })
    end
end
        ...
        for id, name in ipairs(list) do
            if id < #list then
                ...
                if name == '-' or (o.uosc_syntax and name:sub(1, 3) == '---') then
                    ...
                    local shortcut = (key ~= '' and key ~= '_') and ((key == 'SHARP' and '#') or (key == 'Ctrl+_' and '_') or key or nil)
